...
class SkeletonizerContour:
    ...
    def __init__(self, inputImgBinary, colorImg, n_segments_each_skeleton = 15, thres_avg_max_radicle_thickness=12) -> None:
        ...
        self.skeltonized = self.getSkeletonImg()   ## Skeletonized image
    ...
    def get_line_endpoints_intersections(self, skeletonized_img_np_array=None):
        """
        Analyzes the skeletonized image and classifies each white pixel based on
        the number of white pixels in its 3x3 neighborhood.
        Uses vectorized convolution for improved performance and includes
        boundary handling using BORDER_CONSTANT.
        
        Returns:
            A list containing two lists:
            [list_intersection_points, list_line_end_points]
        """
        if skeletonized_img_np_array is None:
            skeletonized_img_np_array = self.skeltonized

        # Convert image to a binary (0/1) format: white pixels (255) become 1.
        binary_norm = (skeletonized_img_np_array == 255).astype(np.uint8)
        
        # Create a 3x3 kernel of ones.
        kernel = np.ones((3, 3), dtype=np.uint8)
        
        # Compute the sum over each 3x3 neighborhood.
        # BORDER_CONSTANT pads the borders with 0 so that all pixels have a full neighborhood.
        neighbor_counts = cv2.filter2D(binary_norm, -1, kernel, borderType=cv2.BORDER_CONSTANT)
        
        # For each white pixel (where binary_norm==1) determine its classification:
        #  - Isolated: neighborhood sum == 1 (only itself)
        #  - Endpoint: neighborhood sum == 2
        #  - Continuous: neighborhood sum == 3
        #  - Intersection: neighborhood sum > 3
        self.list_isolated_points = np.argwhere((binary_norm == 1) & (neighbor_counts == 1)).tolist()
        self.list_line_end_points = np.argwhere((binary_norm == 1) & (neighbor_counts == 2)).tolist()
        self.list_continuous_line_points = np.argwhere((binary_norm == 1) & (neighbor_counts == 3)).tolist()
        self.list_intersetion_points = np.argwhere((binary_norm == 1) & (neighbor_counts > 3)).tolist()
